...
G4bool GmHistoryOrAncestorsFilterForScorer::AcceptStep(const G4Step* aStep)
{
  ...
  if( bPassed.find(trkID) != bPassed.end()  
      || bPassed.find( aTrack->GetParentID()) != bPassed.end()) {
#ifndef GAMOS_NO_VERBOSE
    if( FilterVerb(debugVerb) ) G4cout << theFilterName <<" GmHistoryOrAncestorsFilterForScorer::AcceptStep return 1, because previous step of current track or ancestor was accepted " << G4endl;
#endif
