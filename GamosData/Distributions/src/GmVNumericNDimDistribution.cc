...
{  
  ...
  if( DistVerb(testVerb) ) {
    G4cout << " NDim::BuildData " <<theNDim  << G4endl; //GDEB
  }
#endif
  if(theNDim == 0 ) { 
...
#ifndef GAMOS_NO_ROOT
//---------------------------------------------------------------------
void GmVNumericNDimDistribution::ReadFileROOT( G4String& fileName )
{
...
//---------------------------------------------------------------------
void GmVNumericNDimDistribution::ReadFileCSV( G4String& fileName )
{
  /*
...
void GmVNumericNDimDistribution::ReadFileText( G4String& fileName )
{
...
#ifndef GAMOS_NO_VERBOSE
      if( DistVerb(testVerb) ) {
	G4cout << " aNDim::RFT theIndices " << ii << " "<< id1->size() << "=" << theIndices[ii]->size() << " " << currentIndices[ii] << " Ids " << currentIds[ii] << G4endl; //GDEB
      }
#endif
    }
...
#ifndef GAMOS_NO_VERBOSE
    if( DistVerb(testVerb) ) {
      G4cout << " NDim::RFT NDIM " << ii << " "<< Dims[ii] << G4endl; //GDEB
    }
#endif
  }
#ifndef GAMOS_NO_VERBOSE
  if( DistVerb(testVerb) ) {
    for( size_t ir = 0; ir < IndicesRead.size(); ir++ ){
      for( G4int ii = 0; ii < theNDim; ii++ ){
	G4cout << " NDim::RFT IndicesRead " << ir << ":" << ii << " "<< IndicesRead[ir][ii]  << G4endl; //GDEB
      }
    }
  }
#endif
...
#ifndef GAMOS_NO_VERBOSE
    if( DistVerb(testVerb) ) {
      G4cout << " NDim::RFT CO Dims " << ii << " " << Dims[ii]  << G4endl; //GDEB
    }
#endif
    G4int NFactor = 1;
...
#ifndef GAMOS_NO_VERBOSE
    if( DistVerb(testVerb) ) {
      G4cout << " NDim::RFT CO NFactor " << ii << " " << NFactor  << G4endl; //GDEB
    }
#endif
...
#ifndef GAMOS_NO_VERBOSE
      if( DistVerb(testVerb) ) {
	G4cout << " NDim::RFT CO firstIndex " << ii << " " << iid << " =" << firstIndex << " " << NFactor*iid << G4endl; //GDEB
      }
#endif
      for( size_t ir = 0; ir < IndicesRead.size(); ir++ ){
#ifndef GAMOS_NO_VERBOSE
	if( DistVerb(testVerb) ) {
	  G4cout << " NDim::RFT CO ir " << ir << G4endl; //GDEB
	  G4cout << " NDim::RFT CO check id IndicesRead " << ii << " " << iid << " =? " <<  G4int(ir)/NFactor%Dims[ii] << " " << ir << " / " << NFactor << " % " << Dims[ii] << G4endl; //GDEB
	}
#endif
...
G4double GmVNumericNDimDistribution::GetNumericValueFromIndex(const std::vector<G4double> indexVals)
{
  ...
  for( G4int ii = 0; ii < theNDim; ii++ ) {
#ifndef GAMOS_NO_VERBOSE
    if( DistVerb(testVerb) ) {
      G4cout << " NDim GNVFI indexVals " << ii << " = " << indexVals[ii] << G4endl; //GDEB
    }
#endif
    if( indexVals[ii] < theMinimumNDim[ii] ) {
    ...
    if( indexVals[ii] > theMaximumNDim[ii]*1.000001 ) { // precision problems
      if( bAllowOutOfLimits ) {
	G4Exception(G4String(theName+"::GetIndexFromValue").c_str(),
...
#ifndef GAMOS_NO_VERBOSE
    if( DistVerb(testVerb) ) {
      G4cout << " NDim GNVFI " << ii << " Setting IDDown " << IndicesIdDown[ii] << " IDUp " << IndicesIdUp[ii] << G4endl; //GDEB
...
#ifndef GAMOS_NO_VERBOSE
  if( DistVerb(testVerb) ) {
    G4cout << " NDim GNVFI nIter " << nIter << G4endl; //GDEB
  }
#endif
 ...
 */
    ...
    if( DistVerb(testVerb) ) {
      for( G4int jj = theNDim-1; jj >= 0; jj-- ) {
	G4cout << " 0NDim GNVFI iteLoop " << ii << ":" << jj << " = " << iteLoop[jj] << " + " << IndicesIdDown[jj] << " UP " << IndicesIdUp[jj] <<G4endl; //GDEB
      }
    }
#endif
    std::vector<G4int> indices1;
