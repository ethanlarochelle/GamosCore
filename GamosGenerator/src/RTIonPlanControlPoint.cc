  ...
  : theBeam(beam)
{
  ...
  for(;;) {
   ...
   //------------------------------- parameter string
	...
	G4double ms = GmGenUtils::GetValue(wl[2]);
	theMetersets.push_back(ms);
...
void RTIonPlanControlPoint::RescaleMetersets()
{
  ...
  G4double metersetSC = 0; // sum of metersets of ScanSpots (should be equal to beam meterset)
  for( int ii = 0; ii < nSCs; ii++ ) {
    metersetSC += theMetersets[ii];
  }
  G4double metersetFactor = beamMS / metersetSC;
  G4cout << " metersetFactor " << metersetFactor << " =  " << beamMS << " / " << metersetSC << G4endl; //GDEB
  for( int ii = 0; ii < nSCs; ii++ ) {
    theMetersets[ii] = theMetersets[ii]*metersetFactor;
  }
}
