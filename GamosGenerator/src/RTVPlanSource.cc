...
G4PrimaryVertex* RTVPlanSource::GenerateVertex( G4double time )
{
  if( !bInitialized ) {
    ...
    if( PVs.size() > 0 ) {
      ...
      // G4cout << "1RTVPlanSource::MovePhantom theOrigPhantomTranslation " << theOrigPhantomTranslation  << G4endl;   //GDEB
      bPhantom = true;
    } else {
      bPhantom = false;
    }
  ...
  if( theEventID == theNEventsToSkip // theEventID=0 (or +theNEventsToSkip)
	...
	if( GenerVerb(testVerb) )
	  G4cout << " GOT NEW BEAM spdata "
		 ...
		 << " Beam " << spdata.Beam->GetIndex() << G4endl; 
#endif
...
{
  PredictInitialDisplacement( spdata );
  ...
  rotMatGA.rotateY(spdata.RotAngleY);
#ifndef GAMOS_NO_VERBOSE
  if( GenerVerb(testVerb) ) G4cout << "RTVPlanSource::MovePhantom unit rotMat after rotation Y  " << spdata.RotAngleY/CLHEP::deg << " " << rotMatGA << G4endl;
#endif
  *phantomRotMat *= rotMatGA;
#ifndef GAMOS_NO_VERBOSE
  if( GenerVerb(debugVerb) ) G4cout << "RTVPlanSource::MovePhantom rotmat after rotation Y  " << *phantomRotMat << G4endl;
#endif
  ...
  if( GenerVerb(testVerb) ) G4cout << "RTVPlanSource::MovePhantom FINAL phantomTranslation " <<phantomTranslation  << G4endl
				   << " FINAL phantomRotation " << *phantomRotMat << G4endl;
#endif
...
//------------------------------------------------------------------------
void RTVPlanSource::PredictInitialDisplacement(const ScanSpotData& spdata )
{
  std::vector<G4double> finalPosPar = GmParameterMgr::GetInstance()->GetVNumericValue("RTVPlanSource:FinalPosition", std::vector<G4double>());
  ...
  G4ThreeVector posSAD(0.,0.,spdata.SourceAxisDistance);
  G4RotationMatrix rotMatGA; rotMatGA.rotateY(spdata.RotAngleY);
  ...
  G4cout << " initialDispPREDICT " << initialDispPREDICT << " TO MATCH " << finalPosition << " - " << pcCenter << " - " << posSAD << " - " << isocLocal << G4endl;
