...
void GmModuleMLC::BuildObjects()
{
  ...
  SetWord("LONGHDIM",ip++);  G4double LONGHDIM    = G4tgrUtils::GetDouble(theWords["LONGHDIM"]);
  ...
  if( theWords["LEAFTIPTYPE"] == "ROUND" ){
...
#ifndef GAMOS_NO_VERBOSE
    if( GeomVerb(debugVerb) ) G4cout << " GmModuleMLC::BuildObjects " << PrintWVal("HVL") << G4endl;
#endif  
    ...
    G4Exception("GmModuleMLC:BuildObjects",
		"M001",
		FatalException,
		("MLC TYPE HAS TO BE STRAIGHT, ROUND, ROUND_POSFILE, WHILE IT IS "+theWords["LEAFTIPTYPE"]).c_str());
  }
...
SetWord("N_LEAF_CROSS_PROFILES",ip++); G4double N_LEAF_CROSS_PROFILES= G4tgrUtils::GetDouble(theWords["N_LEAF_CROSS_PROFILES"]);
  ...
  G4String R_90x = "RM90X_MLC";
  G4String R_90x_90y = "RM90X90Y_MLC";
  G4String R_90y = "RM90Y_MLC";
  G4String R_270y_180z = "RM270Y180Z_MLC";
  G4String R_270y = "RM270Y_MLC";
  G4String R00   = "RM0_MLC";
  //  G4String R_270x_90z = "RM270X90Z_MLC";
  G4String R_90x_90z = "RM90X90Z_MLC";
  G4String R_aux;
  ...
  for(int ii=1;ii<int(1+N_LEAF_PAIRS);ii++){
	...
	GmParameterMgr::GetInstance()->AddParam("leafPosMLCDist:FileName "+ theWords["TIP_POS_FILE"]);
	GmParameterMgr::GetInstance()->AddParam("leafPosMLCDist:Data InitialPosX"); // dummy data
...
#ifndef GAMOS_NO_VERBOSE
	if( GeomVerb(testVerb) ) G4cout << " POS_CIRCLE_CENTRE posCN " << posCN << " from opening " << openingNeg << G4endl;
#endif
	posCN *= z_centre/Z_ISOCENTRE;
#ifndef GAMOS_NO_VERBOSE
	if( GeomVerb(testVerb) ) G4cout << " POS_CIRCLE_CENTRE posCN corrected by Z " << posCN << " *= " << z_centre << " / " << Z_ISOCENTRE << G4endl;
#endif
	
	G4double posCP = posLeafDist->GetNumericValueFromIndex(openingPos);
#ifndef GAMOS_NO_VERBOSE
	if( GeomVerb(testVerb) ) G4cout << " POS_CIRCLE_CENTRE posCP " << posCP << " from opening " << openingPos << G4endl;
#endif
	posCP *= z_centre/Z_ISOCENTRE;
#ifndef GAMOS_NO_VERBOSE
	if( GeomVerb(testVerb) ) G4cout << " POS_CIRCLE_CENTRE posCP corrected by Z " << posCP << " *= " << z_centre << " / " << Z_ISOCENTRE << G4endl;
#endif
	fout << ":PLACE " << theWords["NAME"] << "_Leaf_" <<GmGenUtils::itoa(ii) << "_NEG 1 "
	     << theWords["MOTHER_VOLUME"] << " "
	     << R_aux;
	...
	R_aux=R_90x_90z;
      ...
      ///--- LEAVES POS
      ///--- LEAVES NEG
      fout<<":VOLU "<<theWords["NAME"]<<"_Leaf_"<<GmGenUtils::itoa(ii)<<"_NEG  POLYCONE "
	  <<" 270"<<"+("<<theWords["OPEN_POSITION_NEG["+GmGenUtils::itoa(ii)+"]"]<< ")*"<<GmGenUtils::ftoa(Z_TOP/Z_ISOCENTRE)<<"-"<<GmGenUtils::ftoa(1*(180/3.1416)*LONGHDIM*2/(Z_TOP-Z_FOCUS))  <<" "      <<GmGenUtils::ftoa(1*(180/3.1416)*LONGHDIM*2/(Z_TOP-Z_FOCUS))<<" "<<theWords[text_aux0]<<" "
	  <<output_line_text<<" "<<theWords["MATE"]; 
      BuildObject( fout );
      fout << ":PLACE " <<  theWords["NAME"]<<"_Leaf_"<<GmGenUtils::itoa(ii)<<"_NEG 1 " <<  theWords["MOTHER_VOLUME"]<<" "<<R_aux 
	   << " 0 0 0 "; BuildObject(fout );  
      
      fout<<":VOLU "<<theWords["NAME"]<<"_Leaf_"<<GmGenUtils::itoa(ii)<<"_POS  POLYCONE "
	  <<" 270"<<"+("<<theWords["OPEN_POSITION_POS["+GmGenUtils::itoa(ii)+"]"] << ")*"
	  << GmGenUtils::ftoa(Z_TOP/Z_ISOCENTRE) << " "
	  << GmGenUtils::ftoa((180/3.1416)*LONGHDIM*2/(Z_TOP-Z_FOCUS))<<" "
	  << theWords[text_aux0] << " "
	  << output_line_text << " "
	  << theWords["MATE"]; 
      BuildObject( fout );
      fout << ":PLACE " <<  theWords["NAME"]<<"_Leaf_"<<GmGenUtils::itoa(ii)<<"_POS  1 " <<  theWords["MOTHER_VOLUME"]<<" "<<R_aux 
	   << " 0 0 0 "; BuildObject(fout );
...
#endif
  G4String motherVolume = G4tgrVolumeMgr::GetInstance()->GetTopVolume()->GetName();
