...
GmBOptnEWPS::GmBOptnEWPS(G4String name)
: GmVEWBiasingOperation(name)
{
 addPartProcCmd = new GmUIcmdWithAString("/gamos/physics/biasing/EWPS/addParticleProcesses2Oper",this);
  addPartProcCmd->SetGuidance("Add a process of a particle to be biased to EWPS  operator");
...
GmBOptnEWPS::~GmBOptnEWPS()
{
  if (addPartProcCmd) delete addPartProcCmd;
}

//-----------------------------------------------------------------------
void GmBOptnEWPS::SetNewValue(G4UIcommand * command,
						   G4String newValues)    
{ 
  ...
  if( wl[1] == "eBrem" ) {
    ...
    G4Exception("GmBOptnEWPS::SetNewValue",
		"BOEWPS1",
...
void GmBOptnEWPS::SetNSplit(G4int splittingFactor)
{
  std::map<G4String,GmVEWBiasingOperation*>::const_iterator ites;
  //  G4cout << " GmBOptnEWPS::SetNSplit " << theOperationNames.size() << G4endl; //GDEB
...
ApplyFinalStateBiasing( const G4BiasingProcessInterface* callingProcess,
                        ...
                        G4bool&    bval                               )
{
  //  G4cout << " GmBOptnEWPS " << callingProcess->GetProcessName() << G4endl; //GDEB
