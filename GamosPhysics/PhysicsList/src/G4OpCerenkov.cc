...
G4OpCerenkov::G4OpCerenkov(const G4String& processName, G4ProcessType type)
           : G4VProcess(processName, type)
{
        G4cout << "G4OpCerenkov::G4OpCerenkov constructor" << G4endl;
...
// G4OpCerenkov::G4OpCerenkov(const G4OpCerenkov &right)
// {
// }
...
//
G4VParticleChange*
G4OpCerenkov::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
...

{
	...
	G4String calcType = GmParameterMgr::GetInstance()->GetStringValue("G4OpCerenkov:Method","");
	  	...
	  	if( calcType == "fixed" ) {
		    ...
		    G4Exception("G4OpCerenkov:Method",
				"Error in type of calculation", 
				  FatalErrorInArgument,
				  G4String("Please specify with /gamos/setParam G4OpCerenkov:Method TYPE. Only options supported: fixed and interpolate, you have selected " + calcType).c_str());
		}
	...
	for (G4int i = 0; i < NumPhotons; i++) {
		...
		enerprob=enerprobmap[materialIndex];
	    ...
	    if (calcType == "fixed"){
			do {
				...
				std::map<G4double,G4double>::iterator ite = enerprob.lower_bound( pv );
		...
		// G4cout << 1239.84187/sampledEnergy/1000000 << G4endl;
	...
	if (verboseLevel>0) {
	G4cout << "\n Exiting from G4OpCerenkov::DoIt -- NumberOfSecondaries = " 
...
void G4OpCerenkov::BuildThePhysicsTable()
{
	...
	for (G4int ii=0 ; ii < numOfMaterials; ii++)
	{

		// Retrieve vector of refraction indices for the material
		// from the material's optical properties table 

		G4Material* aMaterial = (*theMaterialTable)[ii];
		...
		if (aMaterialPropertiesTable) {
		   ...
		   if (theRefractionIndexVector) {
			   ...
			   for (size_t i = 0; i < theRefractionIndexVector->GetVectorLength(); i++)
			   {
			   		energy = theRefractionIndexVector->Energy(i);
					temprob[energy]=1.0*energy*energy;
			   }
			    ...
			    for(ite = temprob.begin(); ite != temprob.end(); ite++){
				    ...
				    tp += prob * maxProbInv;
				    enerprob[tp] = (*ite).first;
		   		}
		}
	}
	enerprobmap[ii]=enerprob;
  }	
}
...
G4double G4OpCerenkov::GetMeanFreePath(const G4Track&,
                                           G4double,
                                           G4ForceCondition*)
{
        return 1.;
...
G4double G4OpCerenkov::PostStepGetPhysicalInteractionLength(
...
G4double 
G4OpCerenkov::GetAverageNumberOfPhotons(const G4double charge,
                              const G4double beta, 
