...
////////////////////////////////////////////////////////////////////////
//
// File:        G4OpTissueBoundaryProcess.cc
...
G4OpTissueBoundaryProcess::G4OpTissueBoundaryProcess(const G4String& processName,
                                               G4ProcessType type)
...
// G4OpTissueBoundaryProcess::G4OpTissueBoundaryProcess(const G4OpTissueBoundaryProcess &right)
// {
// }
...
G4OpTissueBoundaryProcess::~G4OpTissueBoundaryProcess(){}
...
//
G4VParticleChange*
G4OpTissueBoundaryProcess::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
{
        theStatus = Undefined;
		...
		const G4Material* aMaterial = pPreStepPoint  -> GetMaterial();		
		G4double thePhotonEnergy = aParticle->GetTotalEnergy();		
		G4double AbsorptionLength = DBL_MAX;		
	    G4MaterialPropertiesTable* aMaterialPropertyTable =
	    aMaterial->GetMaterialPropertiesTable();
		
		G4double theThreshold = GmParameterMgr::GetInstance()->GetNumericValue("G4OpTissueOptics:ThresholdWeight",0.0);
		
		if (aMaterialPropertyTable) {
	       G4MaterialPropertyVector* AbsorptionLengthVector =
	                             aMaterialPropertyTable->GetProperty("WEIGHTED_ABSCOEF");
	       if (AbsorptionLengthVector) {
	          AbsorptionLength = AbsorptionLengthVector ->
	                                Value(thePhotonEnergy);
	
      		  G4double steplength = aStep.GetStepLength();
			  G4double newWeight = aTrack.GetWeight()*exp(-steplength/AbsorptionLength);	
			  if ( verboseLevel > 0 ) {
	             G4cout << " Reducing photon weight! " << G4endl;
	             ...
	             if (thePostPV) G4cout << " thePostPV: " << thePostPV->GetName() << G4endl;
		 	     if (AbsorptionLength) G4cout << " theAbsorptionLength: " << AbsorptionLength << G4endl;
		         G4cout << " theStepLength: " << steplength << G4endl;
		         G4cout << " theOldWeight: " << aTrack.GetWeight() << G4endl;
		         G4cout << " theNewWeight: " << newWeight << G4endl;
	          }
	      
	          if (newWeight < theThreshold) {
				 G4double theRoulette=G4UniformRand();
				 if ( verboseLevel > 0 ) {
		             G4cout << "Roulette enabled - rolling number! " << G4endl;
			         G4cout << " theThreshold: " << theThreshold << G4endl;
			         G4cout << " #: " << theRoulette << G4endl;
		         }
				 if (theRoulette < theThreshold) {
					if ( verboseLevel > 0 ) {
			             G4cout << "oldWeight: " << newWeight << G4endl;
				         G4cout << " newWeight: " << newWeight/theThreshold << G4endl;
			        }
					aParticleChange.ProposeWeight(newWeight/theThreshold);
				 }
				 else{
					if ( verboseLevel > 0 ) {
			             G4cout << "Terminating photon!" << G4endl;
			        }
		...
		G4double MieScatteringLength = DBL_MAX;
		
		if (aMaterialPropertyTable) {
	       G4MaterialPropertyVector* MieScatteringLengthVector =
	                             aMaterialPropertyTable->GetProperty("MIE_SCATCOEF");
	       if (MieScatteringLengthVector) {
	          MieScatteringLength = MieScatteringLengthVector ->
	                                Value(thePhotonEnergy);
	       } 
	       else {
	       }
		...
		G4double RayleighScatteringLength = DBL_MAX;

	    if (aMaterialPropertyTable) {
	       G4MaterialPropertyVector* RayleighScatteringLengthVector =
	                             aMaterialPropertyTable->GetProperty("RAYLEIGH_SCATCOEF");
	       if (RayleighScatteringLengthVector) {
	          RayleighScatteringLength = RayleighScatteringLengthVector ->
	                                Value(thePhotonEnergy);
	       } else {
	       }
	    } else {
		...
		G4double UserScatteringLength = DBL_MAX;

	    if (aMaterialPropertyTable) {
	       G4MaterialPropertyVector* UserScatteringLengthVector =
	                             aMaterialPropertyTable->GetProperty("USER_SCATCOEF");
	       if (UserScatteringLengthVector) {
	          UserScatteringLength = UserScatteringLengthVector ->
	                                Value(thePhotonEnergy);
	       } else {
	       }
	    } else {
		...
		G4double TotalScatteringLength = MieScatteringLength + RayleighScatteringLength + UserScatteringLength;
		G4double xo = GmParameterMgr::GetInstance()->GetNumericValue("G4OpRangeReject:xo",0.0);
		G4double yo = GmParameterMgr::GetInstance()->GetNumericValue("G4OpRangeReject:yo",0.0);
		G4double zo = GmParameterMgr::GetInstance()->GetNumericValue("G4OpRangeReject:zo",0.0);
		G4double G = GmParameterMgr::GetInstance()->GetNumericValue("G4OpRangeReject:G",DBL_MAX);
		G4double maxdist = G*(2.0+10.0*TotalScatteringLength);
		G4ThreeVector pos = pPreStepPoint->GetPosition();
		G4double distance = 0.0;
		distance = sqrt(pow((pos(0)-xo),2)+pow((pos(1)-yo),2)+pow((pos(2)-zo),2));
		if(distance > maxdist) {
			if ( verboseLevel > 0 ) {
	             G4cout << "Range Rejection - photon out of range! " << G4endl;
	             G4VPhysicalVolume* thePrePV = pPreStepPoint->GetPhysicalVolume();
	             G4VPhysicalVolume* thePostPV = pPostStepPoint->GetPhysicalVolume();
	             if (thePrePV)  G4cout << " thePrePV:  " << thePrePV->GetName()  << G4endl;
	             if (thePostPV) G4cout << " thePostPV: " << thePostPV->GetName() << G4endl;
		         ...
		         G4cout << " z: " << pos(2) << G4endl;
		 	     G4cout << " theMaxDist: " << maxdist << G4endl;
		         G4cout << " theCurrentDist: " << distance << G4endl;
	        }
	        aParticleChange.ProposeTrackStatus(fStopAndKill);
		}
	...
	OldPolarization   = aParticle->GetPolarization();
        ...
        //ID of navigator which limits steps
        ...
        else 
        {
          ...
          ed << " G4OpTissueBoundaryProcess/PostStepDoIt(): "
                 << " The Navigator reports that it returned an invalid normal"
                 << G4endl;
          G4Exception("G4OpTissueBoundaryProcess::PostStepDoIt", "OpBoun01",
        ...
        if (OldMomentum * theGlobalNormal > 0.0) {
           ...
           ed << G4endl;
           G4Exception("G4OpTissueBoundaryProcess::PostStepDoIt", "OpBoun02",
...
void G4OpTissueBoundaryProcess::BoundaryProcessVerbose() const
{
...
G4ThreeVector
G4OpTissueBoundaryProcess::GetFacetNormal(const G4ThreeVector& Momentum,
...
void G4OpTissueBoundaryProcess::DielectricMetal()
{
        G4int n = 0;
...
void G4OpTissueBoundaryProcess::DielectricLUT()
{
        ...
        theStatus = G4OpTissueBoundaryProcessStatus(G4int(theFinish) + 
...
void G4OpTissueBoundaryProcess::DielectricDielectric()
{
	G4bool Inside = false;
	G4bool Swap = false;
...
// ---------------
//
G4double G4OpTissueBoundaryProcess::GetMeanFreePath(const G4Track& ,
                                              G4double ,
...
G4double G4OpTissueBoundaryProcess::GetIncidentAngle() 
{
    G4double PdotN = OldMomentum * theFacetNormal;
...
G4double G4OpTissueBoundaryProcess::GetReflectivity(G4double E1_perp,
                                              G4double E1_parl,
...
void G4OpTissueBoundaryProcess::CalculateReflectivity()
{
  G4double RealRindex =
