...
// ********************************************************************
//
// $Id: GmVDeexSplitEnergyLossProcess.cc,v 1.19 2016/10/09 18:10:21 arce Exp $
// GEANT4 tag $Name:  $
//
// -------------------------------------------------------------------
//
                                                       ...
                                                       const G4Step& step)
{
  ...
  if(1 < verboseLevel) {
    ...
    G4cout << "AlongStepDoIt for "
           ...
           << d->GetParticleName()
           << "  eScaled(CLHEP::MeV)= " << preStepScaledEnergy/CLHEP::MeV
           << "  range(mm)= " << fRange/mm
           << "  s(mm)= " << length/mm
	   << "  rf= " << reduceFactor
           << "  q^2= " << chargeSqRatio
  ...
  */
  ...
  if (length >= fRange) {
	...
	if(nafter > nbefore) {
	  for (size_t ii2=nbefore; ii2<nafter; ++ii2) {
	    scTracks[ii2]->SetWeight(weight*theNSplitInv);
	  }
	}
      }
  ...
  if(eloss > preStepKinEnergy*linLossLimit) {
    ...
    if(-1 < verboseLevel) 
      G4cout << "Long STEP: rPre(mm)= " 
             ...
             << " rPost(mm)= " << x/mm
             << " ePre(CLHEP::MeV)= " << preStepScaledEnergy/CLHEP::MeV
             << " eloss(CLHEP::MeV)= " << eloss/CLHEP::MeV
             << " eloss0(CLHEP::MeV)= "
             << GetDEDXForScaledEnergy(preStepScaledEnergy)*length/CLHEP::MeV
	     << " lim(CLHEP::MeV)= " << preStepKinEnergy*linLossLimit/CLHEP::MeV
             << G4endl;
    */
  }
  ...
  if(-1 < verboseLevel ) {
    G4cout << "Before fluct: eloss(CLHEP::MeV)= " << eloss/CLHEP::MeV
  ...
  */
  ...
  if (lossFluctuationFlag) {
	...
	std::min(currentModel->MaxSecondaryKinEnergy(dynParticle),cut);
      ...
      /*                            
      if(-1 < verboseLevel) 
      G4cout << "After fluct: eloss(CLHEP::MeV)= " << eloss/CLHEP::MeV
             << " fluc= " << (eloss-eloss0)/CLHEP::MeV
  ...
  /*  
  if(-1 < verboseLevel) {
    G4cout << "Final value eloss(CLHEP::MeV)= " << eloss/CLHEP::MeV
