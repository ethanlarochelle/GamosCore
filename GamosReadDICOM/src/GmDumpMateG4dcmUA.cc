...
if( patientPosition == "HFS" ) {
  ...
  /*
    G4ThreeVector thePhantomMinusCorner = (theRegularParam->GetTranslation(0) + theRegularParam->GetTranslation(theRegularParam->GetNoVoxel()-1) )/ 2;
  ...
  *theFileOut << minZ << " " << maxZ << std::endl;
  ...
  for( size_t iz = 0; iz < nZ; iz++ ) {
    ...
    for( size_t iy = 0; iy < nY; iy++ ) {
      //	  G4cout << ic+ir*fNoVoxelX << " pixdata " << ir << " " << ic << " = " << pixData[ic+ir*fNoVoxelX] << G4endl; //GDEB
