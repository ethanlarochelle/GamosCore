...
G4Material* GmReadPhantomGeometry::BuildMaterialChangingDensity( G4Material* origMate, float density, G4String mateName )
{
  ...
  if( bRecalculateMaterialDensities ) {
    ...
    mate = origMate;
#ifndef GAMOS_NO_VERBOSE
    if( ReadDICOMVerb(debugVerb) ) G4cout << " GmReadPhantomGeometry::BuildMaterialChangingDensity keep original material " << *mate << " density " << density <<  G4endl;
#endif
  }
...
void GmReadPhantomGeometry::ReadVoxelDensities( std::ifstream& fin )
{
  ...
  for( G4int iz = 0; iz < nVoxelZ; iz++ ) {
	...
	if( ReadDICOMVerb(infoVerb) ) {
	  if( copyNo%1000000 == 1 ) G4cout << " Looping ReadVoxelDensities " << copyNo << G4endl; //GDEB
	}
#endif
...
#ifndef GAMOS_NO_VERBOSE
      if( ReadDICOMVerb(debugVerb) ) G4cout << "GmReadPhantomGeometry::ReadVoxelDensities AVER DENS " << averdens << " -> " << saverdens << " -> " << G4int(100000*averdens) << " " << G4int(100000.0001*averdens)/100000 << " " <<  G4int(100000*averdens)/100000. << G4endl;
#endif
