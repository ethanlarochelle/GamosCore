...
std::vector<size_t> GmReadPhantomStMgr::GetStIDList( size_t voxelID )
{
  ...
  //  G4cout << nSTs << " TOO BIG " << nSTs*log10(theROIShift)/log10(2.) << " > " << sizeof(size_t) << G4endl; //GDEB
  if( nSTs*log10(theROIShift)/log10(2.) > 8*sizeof(size_t) ) {
    G4Exception("GmReadPhantomStMgr::GetStIDList",
		"",
		FatalException,
		("Too big number of structures "+GmGenUtils::itoa(nSTs)+" Maximum allowed number is "+GmGenUtils::itoa(int(8*sizeof(size_t)*log10(2.)/log10(theROIShift)))+" . Change the parameter  GmPhantomStructure:ROIShift  to a value smaller than "+GmGenUtils::itoa(int(std::pow(10,8*sizeof(size_t)/nSTs*log10(2.))))).c_str());
  }
