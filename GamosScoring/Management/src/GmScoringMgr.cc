...
void GmScoringMgr::AddPrintByEvent2Printer( const G4String& printerName, G4String& val )   
{
  ...
  std::map<G4String,GmVPSPrinter*>::iterator itepr;
  for( itepr = thePrinters.begin(); itepr != thePrinters.end(); itepr++ ) {
    if( GmGenUtils::AreWordsEquivalent(printerName, (*itepr).first ) ) break;
  }
