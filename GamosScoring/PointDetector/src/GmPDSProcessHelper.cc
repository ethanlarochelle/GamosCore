...
void GmPDSProcessHelper::BookHistos(G4int index)
{
  ...
  if( bControlHistos ) {
	...
	for( size_t kk = 0; kk < 2; kk++ ){ 
	  ...
	  theAnaMgr->CreateHisto1D(hnam,hLogEnerNBins,hLogEnerMin,hLogEnerMax,histoNumber3+1);
	  hnam = hnam0 + G4String("log10(energy) no weighted (MeV)");
	  theAnaMgr->CreateHisto1D(hnam,hLogEnerNBins,hLogEnerMin,hLogEnerMax,histoNumber3+2);
	  hnam = hnam0 + G4String("log10(energy) weighted by Hstar");
	  theAnaMgr->CreateHisto1D(hnam,hLogEnerNBins,hLogEnerMin,hLogEnerMax,histoNumber3+3);
	  
	  hnam = hnam0 + G4String("log10(weight)");
	  theAnaMgr->CreateHisto1D(hnam,hLogWeiNbins,hLogWeiMin,hLogWeiMax,histoNumber3+11);
	  hnam = hnam0 + G4String("log10(weight) weighted by Hstar");
	  theAnaMgr->CreateHisto1D(hnam,hLogWeiNbins,hLogWeiMin,hLogWeiMax,histoNumber3+12);
	  hnam = hnam0 + G4String("log10(energy) vs log10(weight)");
	  theAnaMgr->CreateHisto2D(hnam,hLogEnerNBins,hLogEnerMin,hLogEnerMax,hLogWeiNbins,hLogWeiMin,hLogWeiMax,histoNumber3+21);
	}
      }
    }
  }
