...
void GmPSPrinter3ddoseSplitZ::Write3ddoseHeader(unsigned int iz)
{
 ...
 if( ScoringVerb(infoVerb) ) {
    G4cout << " GmPSPrinter3ddoseSplitZ::Write3ddoseHeader phantom minus corner (temp) " << thePhantomMinusCorner << " " << thePhantomParam->GetTranslation(0) << " " << thePhantomParam->GetTranslation(thePhantomParam->GetNoVoxel()-1) << G4endl;
...
{
  G4int nvoxelsXY = thePhantomParam->GetNoVoxelX() * thePhantomParam->GetNoVoxelY();
