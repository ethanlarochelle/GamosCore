...
{
  theUnit = 1.;
  theUnitName = G4String("CLHEP::MeV/CLHEP::mm");
...
void GmPSLETByESpect::BuildMaterialConversionNames( G4String convFN )
{
  ...
  if( !pathc ) G4Exception("GmPSLETByESpect::BuildMaterialConversionNames",
			   "",
  ...
  convFN = GmGenUtils::FileInPath( path, convFN );
...
{
  char* pathc = getenv( "GAMOS_SEARCH_PATH" );
  if( !pathc ) G4Exception("GmPSLETByESpect::InitialiseE2LET",
			   "",
			   FatalErrorInArgument,
			   "No GAMOS_SEARCH_PATH variable defined, please define it as in config/confgamos.sh or config/confgamos.csh");
  G4String path( getenv( "GAMOS_SEARCH_PATH" ) );
