# - Script for configuring and installing gamos-config script
#
# The gamos-config script provides an sh based interface to provide
# information on the Gamos installation, including installation prefix,
...
function(get_system_include_dirs _dirs)
  ...
  if("${CMAKE_CXX_COMPILER_ID}" MATCHES GNU OR "${CMAKE_CXX_COMPILER_ID}" MATCHES Clang OR "${CMAKE_CXX_COMPILER_ID}" MATCHES Intel)
    # Proceed
    file(WRITE "${CMAKE_BINARY_DIR}/CMakeFiles/gmdummy" "\n")
    ...
    execute_process(COMMAND ${CMAKE_CXX_COMPILER} ${CMAKE_CXX_COMPILER_ARG1} -v -E -x c++ -dD gmdummy
      WORKING_DIRECTORY ${CMAKE_BINARY_DIR}/CMakeFiles
      ERROR_VARIABLE _cxxOutput
      OUTPUT_VARIABLE _cxxStdout 
      )

    file(REMOVE "${CMAKE_BINARY_DIR}/CMakeFiles/gmdummy")
...
#
if(NOT GAMOS_BUILD_GRANULAR_LIBS AND UNIX)
  ...
  if(GAMOS_USE_GDML)
    ...
    set(Gm_XERCESC_INCLUDE_DIRS ${XERCESC_INCLUDE_DIRS})
    list(REMOVE_DUPLICATES Gm_XERCESC_INCLUDE_DIRS)
    list(REMOVE_ITEM Gm_XERCESC_INCLUDE_DIRS ${_cxx_compiler_dirs})

    set(Gm_XERCESC_CFLAGS )
    foreach(_dir ${Gm_XERCESC_INCLUDE_DIRS})
      set(Gm_XERCESC_CFLAGS "${Gm_XERCESC_CFLAGS} -I${_dir}")
    endforeach()
  else()
  ...
  if(GAMOS_USE_QT)
    ...
    set(Gm_QT_INCLUDE_DIRS ${QT_QTCORE_INCLUDE_DIR} ${QT_QTGUI_INCLUDE_DIR} ${QT_QTOPENGL_INCLUDE_DIR})
    list(REMOVE_DUPLICATES Gm_QT_INCLUDE_DIRS)
    list(REMOVE_ITEM Gm_QT_INCLUDE_DIRS ${_cxx_compiler_dirs})

    set(Gm_QT_CFLAGS )
    foreach(_dir ${Gm_QT_INCLUDE_DIRS})
      set(Gm_QT_CFLAGS "${Gm_QT_CFLAGS} -I${_dir}")
    endforeach()
  ...
  if(Gm_CONFIG_NEEDS_X11)
    ...
    foreach(_p ${_raw_x11_includes})
      set(Gm_X11_CFLAGS "-I${_p} ${Gm_X11_CFLAGS}")
    endforeach()
  endif()
  ...
  # paths...
  set(GAMOS_CONFIG_SELF_LOCATION "# BUILD TREE IS NON-RELOCATABLE")
  set(GAMOS_CONFIG_INSTALL_PREFIX "${PROJECT_BINARY_DIR}")
  set(GAMOS_CONFIG_INSTALL_EXECPREFIX \"\")
  set(GAMOS_CONFIG_LIBDIR ${CMAKE_LIBRARY_OUTPUT_DIRECTORY})

  get_property(__gamos_buildtree_include_dirs GLOBAL PROPERTY
    GAMOS_BUILDTREE_INCLUDE_DIRS)

  foreach(_dir ${__gamos_buildtree_include_dirs})
    set(GAMOS_CONFIG_INCLUDE_DIRS "${GAMOS_CONFIG_INCLUDE_DIRS} \\
    ${_dir}")
  endforeach()
  ...
  if(${CMAKE_VERSION} VERSION_GREATER 2.7)
    configure_file(${CMAKE_SOURCE_DIR}/cmake/Templates/gamos-config.in
      ${PROJECT_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/gamos-config
      @ONLY)
    ...
    # that you have to do 'sh gamos-config' in this case.
    configure_file(${CMAKE_SOURCE_DIR}/cmake/Templates/gamos-config.in
      ${PROJECT_BINARY_DIR}/gamos-config
      @ONLY
      )
  endif()
  ...
  if(CMAKE_INSTALL_IS_NONRELOCATABLE)
    # Hardcoded paths
    set(GAMOS_CONFIG_INSTALL_PREFIX "${CMAKE_INSTALL_PREFIX}")
    set(GAMOS_CONFIG_INSTALL_EXECPREFIX \"\")
    set(GAMOS_CONFIG_LIBDIR "${CMAKE_INSTALL_FULL_LIBDIR}")
    set(GAMOS_CONFIG_INCLUDE_DIRS "${CMAKE_INSTALL_FULL_INCLUDEDIR}/Gamos")
  else()
    ...
    set(GAMOS_CONFIG_INSTALL_PREFIX "$scriptloc/${_bin_to_prefix}")
    set(GAMOS_CONFIG_INSTALL_EXECPREFIX \"\")
    set(GAMOS_CONFIG_LIBDIR "\${prefix}/${CMAKE_INSTALL_LIBDIR}")
    set(GAMOS_CONFIG_INCLUDE_DIRS "\${prefix}/${CMAKE_INSTALL_INCLUDEDIR}/Gamos")
  endif() 

  # Configure the install tree script
  configure_file(${CMAKE_SOURCE_DIR}/cmake/Templates/gamos-config.in
    ${PROJECT_BINARY_DIR}/InstallTreeFiles/gamos-config
    @ONLY
    )

  # Install it
  install(FILES ${PROJECT_BINARY_DIR}/InstallTreeFiles/gamos-config
