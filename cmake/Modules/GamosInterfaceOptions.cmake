# - Configure options for Gamos UI/Vis requiring Third Party Libraries
...
#
option(GAMOS_USE_INVENTOR "Build Gamos OpenInventor Visualization Driver" OFF)
...
if(GAMOS_USE_INVENTOR)
  ...
  if(UNIX)
    ...
    if(NOT X11_Xpm_FOUND)
      message(FATAL_ERROR "Could not find X11 Xpm headers and/or library (Required by GAMOS_USE_INVENTOR)")
    endif()
  endif()

  GAMOS_ADD_FEATURE(
    GAMOS_USE_INVENTOR "Build OpenInventor Driver"
    )
endif()
...
#
option(GAMOS_USE_QT "Build Gamos with Qt4 support" OFF)
...
if(GAMOS_USE_QT)
  ...
  # add OpenGL.
  set(GAMOS_USE_OPENGL ON)
  GAMOS_ADD_FEATURE(GAMOS_USE_QT "Build Gamos with Qt support")
endif()
...
if(UNIX)
  ...
  option(GAMOS_USE_NETWORKDAWN "Build Dawn driver with Client/Server support" OFF)
  #
  # Possible headers checks for needed network parts?
  #
  GAMOS_ADD_FEATURE(GAMOS_USE_NETWORKDAWN "Build Dawn driver with Client/Server support")
  mark_as_advanced(GAMOS_USE_NETWORKDAWN)

  # - Support for Client/Server VRML driver
  # mark as advanced because user should know what they're doing to use this
  option(GAMOS_USE_NETWORKVRML "Build VRML driver with Client/Server support" OFF)
  #
  # Possible header checks for needed network parts?
  #
  GAMOS_ADD_FEATURE(GAMOS_USE_NETWORKVRML "Build VRML driver with Client/Server support")
  mark_as_advanced(GAMOS_USE_NETWORKVRML)

  # - Configure Xm support if requested
  option(GAMOS_USE_XM "Build Gamos with Motif (X11) support" OFF)

  # - Configure OpenGL X11 support if requested
  option(GAMOS_USE_OPENGL_X11 "Build Gamos OpenGL driver with X11 support"
    OFF)

  # - X11 Support for RayTracer driver
  option(GAMOS_USE_RAYTRACER_X11 "Build RayTracer driver with X11 support" OFF)
  ...
  # - X11
  if(GAMOS_USE_XM OR GAMOS_USE_OPENGL_X11 OR GAMOS_USE_RAYTRACER_X11 OR GAMOS_USE_INVENTOR)
    find_package(X11 REQUIRED)
    ...
    # Create final list of X11 libraries needed for Gamos
    # - Motif
    if(APPLE)
      if(GAMOS_USE_XM OR GAMOS_USE_INVENTOR)
        set(X11_LIBRARIES ${X11_LIBRARIES} ${X11_Xt_LIB})
      endif()
    endif()

    # - If we got to this point, RayTracer is o.k., so add the feature
    GAMOS_ADD_FEATURE(GAMOS_USE_RAYTRACER_X11 "Build RayTracer driver with X11 support")
  endif()

  #- OpenGL : we also do this for Inventor so that we pick up Mac X11 GL...
  if(GAMOS_USE_INVENTOR OR GAMOS_USE_XM OR GAMOS_USE_OPENGL_X11)
    ...
    GAMOS_ADD_FEATURE(GAMOS_USE_OPENGL_X11 "Build Gamos OpenGL driver with X11 support")
  endif()

  # - Add need for OpenGL in X11 case
  if(GAMOS_USE_OPENGL_X11 OR GAMOS_USE_XM)
    set(GAMOS_USE_OPENGL ON)
  endif()
  ...
  if(GAMOS_USE_XM)
    find_package(Motif REQUIRED)
    GAMOS_ADD_FEATURE(GAMOS_USE_XM "Build Gamos with Xm Support")
  endif()  
endif()
...
if(WIN32)
  # - OpenGL Win32...
  option(GAMOS_USE_OPENGL_WIN32 "Build OpenGL driver with Win32 support")

  if(GAMOS_USE_OPENGL_WIN32)
    # Just need OpenGL and on Windows, this should be easy...
    find_package(OpenGL REQUIRED)

    # This is part of the GmOpenGL component, so mark it as needed
    set(GAMOS_USE_OPENGL ON)
    GAMOS_ADD_FEATURE(
      GAMOS_USE_OPENGL_WIN32 "Build OpenGL driver with Win32 support"
      )
  endif()
endif()
